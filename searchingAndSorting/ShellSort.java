...
public class BubbleSort{
	...
	*  Description: Takes in parameters that are used to organize	**
	* 			    the inputed array in the designated order by    **
	* 				taking out values at intervals and assessing 	**
	* 			 	and putting it in the right position			**
	*****************************************************************/
	...
	public static int[] shellSort(int array[], int len, int direction) {
		...
		for(interval = len/2; interval > 0; interval /=2) {
			for(int x = interval; x < len; x++) {
				...
				int pos = 0;							//create a variable that loops the for loop and tracks what pos in the array you are at
				
				for(pos = x; direction == 0 && (pos >= interval && array[pos-interval] > temp) || direction == 1 && (pos >= interval && array[pos-interval] < temp); pos -= interval) {
					
					array[pos] = array[pos-interval];	//swap them 
					
				} //end for loop that loops to find the correct pos to put the low number
				array[pos] = temp;						//put it in the correct spot
			} //end for loop that rearranges at a smaller range 
		} //end for loop that rearranges the values at large range
	...
	**********************************************************/
	...
	public static void main(String[] args) throws IOException{
		...
		int[] shellArray = new int[99999];																	//create array to hold shell sort output
        ...
        printReportTitle("Bubble Sort, Insertion Sort & Shell Sort");  	    								//send the title to the printReportTitle method
		...
		try {
            ...
            while((inputData = inputFile.readLine()) != null) {
            	...
            	shellArray[length] = Integer.parseInt(inputData);											//parse inputData into shell sort
            ...
            startTime = System.nanoTime();																	//get starting time 
            shellArray = shellSort(shellArray, length, order);												//set array = to shell sorted data
            endTime = System.nanoTime();																	//get ending time after running the shell sort 
            System.out.println("Shell Sort - Time elapsed: \t"+ (endTime - startTime) + "\n");				//print the time elapsed
            
            outputFile.println("\tBubble Sort Output\t");													//output to file that the output is bubble sorted
            for(int x = 0; x < length; x++) outputFile.println(bubArray[x]);								//for loop that unloads bubble sort into a text file
            ...
            for(int x = 0; x < length; x++) outputFile.println(insArray[x]);								//for loop that unloads insertion sort into a text file 
            System.out.println("Processed insertion sort data into file");									//print current step
            
            outputFile.println("\tShell Sort Output\t");													//output to file that the output is shell sorted
            for(int x = 0; x < length; x++) outputFile.println(shellArray[x]);								//for loop that unloads shell sort into a text file 
            System.out.println("Processed shell sort data into file");										//print current step
